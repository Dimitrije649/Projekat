...
class MainEngine(object):
    ...
    def __initLevel(self, level):
        ...
        field.gameEngine.levelGenerate(level)   
    ...
    def loopFunction(self):
        ...
        for i in range(4):
            coordGhosts['RelG{}'.format(i+1)] = field.gameEngine.movingObjectGhosts[i].coordinateRel  
            coordGhosts['AbsG{}'.format(i+1)] = field.gameEngine.movingObjectGhosts[i].coordinateAbs    
    ...
    def spriteGhost(self, coordGhosts):
        for ghostNo in range(4):
            if field.gameEngine.movingObjectGhosts[ghostNo].isActive == True:   
    ...
    def encounterEvent(self, coordRelP, coordAbsP):
        ...
        encounterMov = field.gameEngine.encounterMoving(coordAbsP[0], coordAbsP[1]) 
        ...
        if coordAbsP[0] % 4 == 0 and coordAbsP[1] % 4 == 0:
            encounterFix = field.gameEngine.encounterFixed(coordRelP[0], coordRelP[1]) 
            ...
            if encounterFix == "empty":
                ...
                if field.gameEngine.levelObjects[coordRelP[0]][coordRelP[1]].isDestroyed == False:  
                    ...
                    self.wGameCanv.itemconfigure(self.wGameCanvObjects[coordRelP[0]][coordRelP[1]], state='hidden')
                    ...
                    field.gameEngine.levelPelletRemaining -= 1 

                    if field.gameEngine.levelPelletRemaining == 0:
                        self.encounterEventLevelClear() 
                    else:
                        pass
    ...
    def encounterEventDead(self):
        ...
        if self.statusLife >= 0:
            self.wGameLabelLife.configure(text=("Life: " + str(self.statusLife))) 
        else: 
            pass
    ...
    def encounterEventDeadRestart(self):
        if self.statusLife >= 0:
            ...
            self.isPlaying = False      
            field.gameEngine.levelPelletRemaining = 0  
            self.__initLevel(self.currentLv)
